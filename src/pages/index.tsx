...
const navigation = [
    { name: 'Projects', href: 'https://www.artstation.com/theappleisalie', target: "blank", className: "text-2xl font-ButtonFont text-gray-200" },
    { name: 'Experience', href: '#', className: "hidden text-2xl font-ButtonFont text-gray-500 cursor-not-allowed" },
    { name: 'Education', href: '#', className: "hidden text-2xl font-ButtonFont text-gray-500 cursor-not-allowed" },
    { name: 'Contact', href: 'mailto:gwenchaves8@gmail.com', target: "blank", className: "text-2xl font-ButtonFont text-gray-200" },
]
    ...
    return (
        ...
        <div className="z-0 relative bg-bp_back bg-[url(/bg_tex.svg)] bg-repeat bg-[_5rem] overflow-hidden">
            ...
            */}
                ...
                <header className="fixed inset-x-0 top-0 z-50"> {/*changed from absolute to fixed*/}
                    <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
                        <div className="flex lg:flex-1">
                            <a href="https://github.com/GwenChaves/GwenChaves.github.io" target="blank" className="-m-1.5 p-1.5">
                        ...
                        <div className="hidden lg:flex lg:gap-x-24 mr-40 mt-8">
                            {navigation.map((item) => (
                                <a key={item.name} href={item.href} target={item.target} className={item.className}>
                                    {item.name}
                                </a>
                            ))}
                        </div>
                ...
                <div className="relative mx-auto max-w-2xl pt-24 pb-16">
                    ...
                    <div className="z-10 relative h-[15rem]">
                        {/*<div className="absolute top-1/1 left-1/2 -translate-x-1/2 -translate-y-1/1 h-[20vh] w-screen overflow-visible">
                            */}
                        <div className="relative -top-[14rem] left-1/2 -translate-x-1/2 bg-[url(/linetrace_bp.svg)] bg-no-repeat bg-auto bg-[left_-340px_center] h-[90rem] aspect-video">
                            &nbsp;
                        </div>
                        {/*</div>*/}
                    </div>
                    ...
                    <div className="hidden z-40 relative justify-self-center -right-[28rem] rounded-lg mt-4 mb-4 px-3 py-1 backdrop-blur-sm bg-gray-400/10 text-3xl text-gray-200 font-ButtonFont ring-1 ring-grad2 hover:ring-grad1">
                        Bringing ideas to{' '}
            ...
            {/*Section Break*/}
            <div className="hidden z-40 bg-bp_back bg-[url(/SectionBreak.svg)] bg-cover bg-center bg-no-repeat min-h-6em h-6em w-svw overflow-visible">
                &nbsp;
            </div>

            {/*Content Section*/}
            <div className="hidden h-svh overflow-hidden">
